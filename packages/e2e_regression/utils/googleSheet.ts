...
async function processResponseForSheet(rows: ResultRow[], startRow: number): Promise<any[][]> {
    ...
    const promises = rows.map(async (r, index) => {
        ...
        const rowData: SheetRow = {
            ...
            dumpedPayload: r.dumpedPayload ?? '',
            llmPrompt: r.llmPrompt ?? '',
            weatherDataPayload: r.weatherDataPayload ?? '',
            payloadJudgeVerdict: r.payloadJudgeVerdict ?? '',
            payloadJudgeScore: r.payloadJudgeScore ?? '',
            payloadExpectedIntent: r.payloadExpectedIntent ?? '',
            payloadActualIntent: r.payloadActualIntent ?? '',
            payloadJudgeChecks: r.payloadJudgeChecks ?? '',
            payloadJudgeSummary: r.payloadJudgeSummary ?? '',
            payloadJudgeIssues: r.payloadJudgeIssues ?? '',
            payloadJudgeError: r.payloadJudgeError ?? '',
